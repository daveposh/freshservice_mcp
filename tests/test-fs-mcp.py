...
async def test_create_requester():
    ...
    result = await create_requester(
        first_name=str(first_name), primary_email=primary_email
    )
    print(result)
...
async def test_get_agent_group_by_id():
    id = 27000298443
...
async def test_create_solution_folder():
    ...
    result = await create_solution_folder(
        name=name, category_id=category_id, department_ids=department_ids
    )
    print(result)
    # 27000183948
